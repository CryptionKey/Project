...
public class Test_Produit {
    ...
    public void test_getIndexAliment(Produits produits){
        logger.info("PROGRAM", "Nom de l'aliment dont on veut récupérer l'index: \"burger\"\nIndex attendu: 0\n");
        int index = Produits.getIndexAliment("burger", produits.getProductList());
        if (index != 0) {
            throw new AssertionError("L'index devrait être 0 mais c'est " + index);
        } else logger.info("PROGRAM","Le test de la fonction \"getIndexAliment\" a réussi\n");
    }
    ...
    public void test_verif_aliment_existant(Produits produits){
        logger.info("PROGRAM", "Nom de l'aliment dont on veut savoir s'il est présent dans la liste: \"burger\"\nRésultat attendu: l'aliment est présent dans la liste\n");
        Aliment aliment = Produits.verification_aliment_existant("burger", false, produits.getProductList());
        if(aliment==null){
            throw new AssertionError("L'aliment \"burger\" est censé se trouver dans la liste de produits");
        }
        else logger.info("PROGRAM", "L'aliment \"burger\" a bien été trouvé dans la liste de produits");
    }

    //Méthode qui teste la vérification de l'existence d'un aliment dans une liste
    public void test_verif_aliment_inexistant(Produits produits){
        logger.info("PROGRAM", "Nom de l'aliment dont on veut savoir s'il est présent dans la liste: \"coca\"\nRésultat attendu: l'aliment n'est pas présent dans la liste\n");
        Aliment aliment = Produits.verification_aliment_existant("coca", false, produits.getProductList());
        if(aliment!=null){
            throw new AssertionError("L'aliment \"coca\" n'est pas censé se trouver dans la liste de produits");
        }
        else logger.info("PROGRAM", "L'aliment \"coca\" n'a pas été trouvé dans la liste de produits");
    }
